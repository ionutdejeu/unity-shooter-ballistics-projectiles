		...
		public virtual void FireAtPoint(Vector3 startPoint, Vector3 targetPoint)
		{
			...
			switch (fireMode)
			{
				case BallisticFireMode.UseLaunchSpeed:
					firingVector =
						BallisticHelper.CalculateBallisticFireVectorFromVelocity(startPoint, targetPoint, startSpeed, arcPreference);
					firingAngle = BallisticHelper.CalculateBallisticFireAngle(startPoint, targetPoint, startSpeed, arcPreference);
					break;
				case BallisticFireMode.UseLaunchAngle:
					firingVector = BallisticHelper.CalculateBallisticFireVectorFromAngle(startPoint, targetPoint, firingAngle);
